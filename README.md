...
The code for this can be found in **camera_calibration.py**. As discussed in the lecture, I used *findChessboardCorners* 
on all of the images to get a mapping between corners found and object points, then I used *calibrateCamera* to find 
...
After I un-distort the image, I threshold the image to extract a binary image showing only lane lines. 
...
* Gradient in x and y direction using Sobel operator (*abs_sobel_thresh*)
* Gradient magnitude threshold (*mag_thresh*)
* Gradient direction threshold (*dir_threshold*)
* RGB thresholding to extract yellow color (*create_binary_image*)
...
It uses sliding window over the image and uses the sum histogram of pixel values to find the lane lines.
...
Then I ignore the lines if the *slope_diff* is greater than 0.1
...
The code below which is part of method **_compute_curvature** in *find_lanes.py* does that.
 
```python
curve_rad = ((1 + (2 * fit[0] * y_eval + fit[1]) ** 2) ** 1.5) / np.absolute(2 * fit[0])
```

To compute the vehicle's distance to the center line I use the following code:
...
in an instance of a **Line** object. Having this, for each frame I compute a moving average
 of the fit parameters for the previous frames as shown in the following code:
...
were terrible. Partly because of the fact that the algorithm is not robust when thresholding shadowy areas. This causes many 
